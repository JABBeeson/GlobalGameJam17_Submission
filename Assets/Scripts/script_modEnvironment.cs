...
public class script_modEnvironment : MonoBehaviour
{
    public float baseHeight = 0.1f;
    public GameObject[] effectedObjects;
    public GameObject[] bases;

    private script_avgFrequency _avgFreq;

    private void Start()
    {
        _avgFreq = GetComponent<script_avgFrequency>();
    ...
    private void Update()
    {
        ...
        if (_avgFreq)
        {
            ...
            if ((timer >= 7.5f) || (timer >= 5.0f && modifierValue >= 0.5f))
            {
                ...
                {
                    script_spawner s = g.GetComponent<script_spawner>();
