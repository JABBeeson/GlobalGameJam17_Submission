...
public class script_junction : script_waypoint
{
    public Object switchPrefab;
    public script_waypoint[] options;
    public int _optionIndex = 0;
    public script_junction previous;
    ...
    private void Update()
    {
        if (Vector3.Distance(script_Manager.Instance.GetPlayer(teamIndex).transform.position, transform.position) < 5.0f)
        {
    ...
    public void Switch()
    {
        ...
        switches[tmp].GetComponent<Renderer>().material.SetColor("_EmissionColor", script_Manager.Instance.GetTeamColor(teamIndex) * 3.0f);
    ...
    public override void CascadeTeam(int index)
    {
        ...
        for (int i = 0; i < options.Length; i++)
        {
            if (options[i])
            {
                ...
                if (options[i] is script_junction && this is script_junction)
                    (options[i] as script_junction).previous = this;
            }
        }
    }
    ...
    protected override void SetSwitches()
    {
        if (switchPrefab)
        {
            ...
            switches[_optionIndex].GetComponent<Renderer>().material.SetColor("_EmissionColor", script_Manager.Instance.GetTeamColor(teamIndex) * 3.0f);
        }
    }
}
